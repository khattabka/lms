...
const DescriptionForm = ({courseId,course}:DescriptionFormProps) => {
...
// * FORM STATES
  ...
  // * DESCRIPTION CHNAGE API MUTATION
  const utils = api.useUtils();
  const { mutate, status } = api.course.updateCourseDescription.useMutation({
      onSuccess: (data: Course) => {
        toast("Course description updated", {
          position: "top-center",
          icon: "👏",
        });
  ...
  return (
    <div className="mt-6 border bg-muted  py-6 px-1 rounded-md">
      ...
      {
        !editing &&(
           <p className={cn("mt-2 text-sm text-muted-foreground", !course?.description && "text-center italic line-clamp-3")}>{course?.description}</p>)
      }
      {editing && (
            <div>
            <Form {...form}>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea
                          ...
                          placeholder={course?.description || "Enter description"}
                          {...field}
                        />
                      </FormControl>
